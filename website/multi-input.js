class MultiInput extends HTMLElement {
  constructor() {
    super();
 	//the 'x' was killing UTF-8 compat for some reason, replaced with a standard character x - JDLS
    this.innerHTML +=
    `<style>
...
//Public method for _refreshing_ the item values
 refresh(){
    this._datalist = this.querySelector('datalist');
    this._allowedValues = [];
    for (const option of this._datalist.options) {
		console.log("pushing values" + option.value);
	this._allowedValues.push(option.value);
    }
 }
