...
class Connection_Node extends Context_Node{
    constructor(parent,server,auth_params){
		...
		var serverLoc = this.connect.server;
		var tokenAuth;
		//TODO - needs fix
		if(serverLoc.includes('sorghum')){
			tokenAuth=serverLoc.slice(0,this.connect.server.lastIndexOf("sorghum"));
		}
		if(serverLoc.includes('cowpea')){
			tokenAuth=serverLoc.slice(0,this.connect.server.lastIndexOf("cowpea"));
		}
		else tokenAuth=serverLoc;
        var requrl = tokenAuth+"/token";
        var self = this;
        if (auth_params){
            fetchRef(requrl, {
                    method: 'post',
    ...
    constructor(parent,connect,behavior,httpMethod,url_body_func,multicall){
        ...
        hookTo.call(parent,function(dat, key){
            ...
            var fetch_args = {
                ...
                headers: {
                    'Content-Type': 'application/json;charset=utf-8',
					'X-Auth-Token': d_call.params.access_token
                }
            };
            key = multicall ? key : 0;
